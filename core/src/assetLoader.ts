/**
 * Asset Loader — loads furniture, floor, wall, and character assets from the
 * filesystem. VS Code-free: send* functions target MessageSender instead of
 * vscode.Webview so the same module feeds the CLI host (WebSocket broadcast).
 *
 * Furniture format: per-folder manifest.json + PNGs under assets/furniture/.
 */
...
/** Layout file key — duplicated from src/constants.ts; keep in sync. */
...
export function sendAssetsToWebview(messageSender: MessageSender, assets: LoadedAssets): void {
  if (!assets) {
