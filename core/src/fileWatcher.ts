 ...
 * - 1s main scanner for terminal adoption (when processProvider supplied)
...
const CLEAR_IDLE_THRESHOLD_MS = 3000;
const DISMISSED_COOLDOWN_MS = 30000;
const EXTERNAL_ACTIVE_THRESHOLD_MS = 60000;
const EXTERNAL_SCAN_INTERVAL_MS = 3000;
const EXTERNAL_STALE_CHECK_INTERVAL_MS = 30000;
const FILE_WATCHER_POLL_INTERVAL_MS = 500;
const GLOBAL_SCAN_ACTIVE_MAX_AGE_MS = 600000;
const GLOBAL_SCAN_ACTIVE_MIN_SIZE = 3072;
const PROJECT_SCAN_INTERVAL_MS = 1000;

const debug = process.env.PIXEL_AGENTS_DEBUG !== '0';

import type { AgentProcess, MessageSender, ProcessProvider } from './interfaces.js';
import { cancelPermissionTimer, cancelWaitingTimer, clearAgentActivity } from './timerManager.js';
import { processTranscriptLine } from './transcriptParser.js';
import type { AgentState } from './types.js';

/** Minimal TeamProvider interface — subset needed by fileWatcher.
 *  Avoid importing server's full TeamProvider to keep core decoupled. */
export interface TeamProviderLike {
  resolveTeammateMetadataPath(jsonlFile: string): string;
  parseTeammateMetadata(content: string): string | null;
  resolveTeammateJsonlDir(projectDir: string, sessionId: string): string;
  getTeamMembers(teamName: string): Set<string> | null;
}

/** removeAgent callback — injected to avoid circular dep with agentManager. */
export type RemoveAgentFn = (
  id: number,
...
let removeAgentFn: RemoveAgentFn | null = null;

/** Register the removeAgent implementation. Called once by the host adapter.
 *  Breaks the circular dep between fileWatcher and agentManager. */
export function setRemoveAgentFn(fn: RemoveAgentFn): void {
  removeAgentFn = fn;
}
...
export function startFileWatching(
  ...
  const interval = setInterval(() => {
    ...
    readNewLines(agentId, agents, waitingTimers, permissionTimers, messageSender);
  ...
  try {
    ...
    if (hasLines) {
      ...
      if (agent.permissionSent && !agent.hookDelivered && !agent.leadAgentId) {
        ...
        messageSender?.postMessage({ type: 'agentToolPermissionClear', id: agentId });
      }
    }

    for (const line of lines) {
      if (!line.trim()) continue;
      processTranscriptLine(agentId, line, agents, waitingTimers, permissionTimers, messageSender);
    }
  } catch (e) {
...
/** Optional adoption filter for the main scanner.
 *  VS Code passes TERMINAL_NAME_PREFIX to adopt only Claude-named terminals.
 *  CLI/headless hosts pass undefined to disable terminal adoption entirely. */
export interface AdoptionConfig {
  processProvider: ProcessProvider;
  /** Only adopt processes whose name starts with this prefix. */
...
export function ensureProjectScan(
  ...
  projectScanTimerRef.current = setInterval(() => {
    ...
    // No adoption configured (e.g. headless CLI host): skip the scan.
    if (!adoption) return;
...
function scanForNewJsonlFiles(
  ...
  for (const file of files) {
    ...
    // Try to adopt the focused process (only if its name matches the prefix)
    const activeProcess = adoption.processProvider.getActive();
    if (activeProcess && activeProcess.name.startsWith(adoption.processNamePrefix)) {
      let owned = false;
      for (const agent of agents.values()) {
        if (agent.processRef === activeProcess) {
          owned = true;
          break;
        }
      }
      if (!owned) {
        knownJsonlFiles.add(file); // Claimed by process adoption
        adoptProcessForFile(
          activeProcess,
          file,
          projectDir,
        ...
        // Active process is owned -- scan for untracked prefix-named processes.
        // Only adopt processes starting with the configured prefix to avoid grabbing
        // pre-existing shells ("zsh", "bash") for /clear files.
        for (const proc of adoption.processProvider.listAll()) {
          if (!proc.name.startsWith(adoption.processNamePrefix)) continue;
          let procOwned = false;
          for (const agent of agents.values()) {
            if (agent.processRef === proc) {
              procOwned = true;
              break;
            }
          }
  ...
  // Clean up orphaned agents whose processes have exited (skip external agents)
  for (const [id, agent] of agents) {
    if (agent.isExternal) continue;
    if (agent.processRef?.exited) {
      console.log(`[Pixel Agents] Watcher: Agent ${id} - process exited, cleaning up orphan`);
      // Stop file watching
...
function adoptProcessForFile(
  ...
  // Skip to end of file -- adopted processes show live activity only, not replay history
  let fileOffset = 0;
  try {
  ...
  console.log(
    `[Pixel Agents] Watcher: Agent ${id} - adopted process "${proc.name}" for ${path.basename(jsonlFile)}`,
  );
  messageSender?.postMessage({ type: 'agentCreated', id });
  ...
  readNewLines(id, agents, waitingTimers, permissionTimers, messageSender);
}
...
let teamProvider: TeamProviderLike | null = null;
...
export function setTeamProvider(provider: TeamProviderLike): void {
...
export function scanForTeammateFiles(
  ...
  for (const file of files) {
    ...
    if (existingTeammate) {
      ...
      readNewLines(existingTeammate.id, agents, waitingTimers, permissionTimers, messageSender);
      continue;
    }
...
export function adoptExternalSessionFromHook(
  ...
  if (transcriptPath) {
    ...
    messageSender?.postMessage({ type: 'agentCreated', id, folderName });
    onAgentCreated?.(agent);
  }
}
...
function adoptExternalSession(
  ...
  messageSender?.postMessage({ type: 'agentCreated', id, isExternal: true, folderName });
